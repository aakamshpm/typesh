...
suite("KeystrokeCaptureService Tests", () => {
  ...
  test("should handle default configuration", () => {
    const defaultService = new KeystrokeCaptureService();

    assert.ok(defaultService, "Service should be created with defaults");
    assert.strictEqual(
      defaultService.getHandlerCount(),
      0,
      "Should start with no handlers"
    );

    defaultService.stopCapture();
  });
  ...
  suite("Integration", () => {
    test("should work with TypingSessionManager", () => {
      ...
      service.addHandlers(keystrokeHandler);
      service.addFocusHandler(focusHandler);
      service.addBlurHandler(blurHandler);
      service.startCapture();
